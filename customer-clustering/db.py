...
class CustomerClusteringDB:
    ...
    def load_bronze_data(self, csv_path: str, chunk_size: int = 50000):
        ...
        for chunk_num, chunk in enumerate(pd.read_csv(csv_path, chunksize=chunk_size)):
            ...
            # Deduplicate on primary_key before insert
            chunk_processed = chunk_processed.drop_duplicates(subset=['primary_key'], keep='last')

            # Print current vs expected column order for the first chunk, just before insertion
            if chunk_num == 0:
                expected_columns = [
                    'primary_key', 'sales_order_no', 'customer_emailid', 'order_date', 'sku', 'sales_qty',
                    'gross', 'return_qty', 'units_returned_flag', 'return_date', 'return_no', 'return_comment',
                    'orderlink', 'q_cls_id', 'q_sku_desc', 'q_gmm_id', 'q_sku_id', 'class_',
                    'division_', 'brand_', 'q_clr_dnum', 'q_clr_desc', 'vendor_style', 'size_'
                ]
                print("Current DataFrame columns before insert:", list(chunk_processed.columns))
                print("Expected table columns:", expected_columns)
    ...
    def _clean_bronze_data(self, df: pd.DataFrame, issues_list: List) -> pd.DataFrame:
        ...
        # Create a more robust primary key using Q_SKU_ID, SALES_ORDER_NO, and CUSTOMER_EMAILID
        df_clean['primary_key'] = (
            df_clean['Q_SKU_ID'].astype(str) + '_' +
            df_clean['SALES_ORDER_NO'].astype(str) + '_' +
        ...
        # Add missing columns as NA if any are missing
        ...
        # After all cleaning and reordering, drop duplicate primary keys (keep last)
