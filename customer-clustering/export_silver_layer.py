"""
Export Silver Layer to XLSX or CSV
This script exports the silver_customer_features table to either XLSX or CSV
...
def check_and_export_silver_layer():
    """Check silver layer size and export to appropriate format"""
    try:
        # Connect to database
        ...
        logger.info(f"Silver layer contains {row_count} rows")
        ...
        if row_count <= MAX_EXCEL_ROWS:
            # Export to Excel
            export_path = OUTPUT_DIR / f"silver_customer_features_{TIMESTAMP}.xlsx"
            ...
            # Export to CSV
            export_path = OUTPUT_DIR / f"silver_customer_features_{TIMESTAMP}.csv"
...
def main():
    ...
    try:
        logger.info("Starting silver layer export process")
        output_file = check_and_export_silver_layer()
